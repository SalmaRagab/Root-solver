%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Trying to find roots of multiplicity%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
function [column_names,interval, res_table,it_pres_root , exec_t, plot, err_message] = brent_fzero_m (func) 
...
t = 1;
tic;
       [column_names,interval, res_table,it_pres_root ,root, exec_t, plot,err_message] = brent_fzero(func);
        sym_func = sym(func);
        root__m = root;
        while t < max_it
        ...
        for i = 1: (size(root))
            ...
            def = sym('x + 0') + subs(root(i));
            sym_func = sym(sym_func / def);
 
        end; 
              try
              bool_brk =  strcmp(num2str(eval(subs(sym_func,0.0998))),num2str(1.000000000000000));
              catch 
                  bool_brk = true;
              end;
             if bool_brk
                ...
                [column_names_1,interval_1, res_table_1,it_pres_root_1 ,root, exec_t, plot,err_message] = brent_fzero(char(sym_func));
                if size(root) == 0
                    %break;
                else
                    interval = [interval_1, interval];
                    res_table = [res_table_1, res_table];
                    it_pres_root = [it_pres_root_1, it_pres_root];
                    root__m = [root__m; root];
                end;
            end;
       t = t + 1;
        end;
        ...
        elseif t == max_it
            err_message = 'Some Roots (Maybe Complex) can not be Evaluated.';
        else 
             interval = interval';
             res_table = res_table';
             it_pres_root = it_pres_root';
             root__m = root__m';
             err_message = '';    
        end;
        exec_t = toc;
...
while fb ~= 0
   ...
   if (abs(m) <= tol) || (fb == 0.0)
       ...
       res_table(tot_it, 1:4) = [tot_it;a;b; abs(m)];
   ...
   res_table(tot_it, 1:4) = [tot_it;x1;x2; abs(x2 - x1)];
